...
// Define an interface for currency rates to ensure type safety
...
async function main() {
  ...
  while (!exit) {
    try {
      ...
      if (user_answer.from === user_answer.to) {
        console.log(
          chalk.yellow("Please select different currencies for conversion.")
        );
      } else {
        ...
        console.log(
          chalk.blue(
            `Converted amount: ${convertedAmount.toFixed(2)} ${user_answer.to}`
          )
        ); // Display the results
      }
  ...
  console.log(
    chalk.green("Thank you for using the Currency Converter! Goodbye!")
  ); // Farewell message
}
